...
description: Overview of messages in Kameo
---

In the Kameo actor framework, messages play a central role in the communication between actors. They are the primary means through which actors interact and modify each other's state. Understanding how messages work is fundamental to effectively using Kameo to build concurrent applications.
